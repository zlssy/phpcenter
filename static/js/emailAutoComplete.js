...
function EmailAutoComplete(options) {
    ...
    this.config = {
        ...
        mailArr        : ["@tcl.com","@kuyumall.com"], //邮箱数组
    ...
    init: function(options){
        ...
        $(_config.targetCls).each(function(index,item){
            $(item).on('focus',function(e){
                ...
                $(item).on('keyup',function(e){
                    var target = e.target,
                        targetVal = $.trim($(this).val()).replace(/\s+/g,""),
                        keycode = e.keyCode,
        ...
        $(document).click(function(e){
            ...
            var target = e.target,
                ...
                panelIsShow = $(".auto-tip").size() && !$(".auto-tip").hasClass("hidden");
        ...
        $(document).keyup(function(e){
            if(e.keyCode == 9){
                var panelIsShow = $(".auto-tip").size() && !$(".auto-tip").hasClass("hidden");
                if(panelIsShow){
                    $('.auto-tip') && $('.auto-tip').each(function(index,item){
                        !$(item).hasClass('hidden') && $(item).addClass('hidden');
                    });
    ...
    var eac = new EmailAutoComplete({
        callback: function(){
           //ajax 校验email
                ...
                var inputItem = $(_config.targetCls);
                login_form.find("li.error_message").hide();
                _config.is_focus = 0;
               
               if(login_form.find("input[name=_is_mobile]").size()){
                   _is_mobile = login_form.find("input[name=_is_mobile]").val();
               }

                $.post(G_BASE_URL + '/account/ajax/login_check_email/',
                    {
                        email: inputItem.val(),
                        ...
                        switch (result.errno) {
                            case -1:
                                inputItem.closest("li").siblings().show();
                                login_form.find("#reset_pwd").hide();
                                login_form.find("#verify_email").hide();
                                login_form.find(".error_message").hide();
                                login_form.find("li.error_message em").text(result.err);
                                login_form.find("li.error_message").show();
                                break;
                            case 0:
                                inputItem.closest("li").siblings().hide();
                                login_form.find("#verify_email").show();
                                login_form.find("li.error_message em").text(result.err);
                                login_form.find("li.error_message").show();
                                break;
                            case 1:
                                inputItem.closest("li").siblings().hide();
                                login_form.find("#reset_pwd").show();
                                login_form.find("li.error_message em").html(result.err);
                                login_form.find("li.error_message").show();
                                break;
                            case 2:
                                inputItem.closest("li").siblings().show();
